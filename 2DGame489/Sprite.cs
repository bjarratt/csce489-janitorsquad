...
namespace _2DGame489
{
    class Sprite
    {
        ...
        public void Update(GameTime theGameTime, Vector2 theSpeed, Vector2 theDirection)
        {
            ...
            if ((Position.X + Size.Width) >= MAX_WINX && is_projectile == false)
                Position.X = MAX_WINX - Size.Width;
            if (Position.X <= 0 && is_projectile == false)
                Position.X = 0;
            if (Position.Y <= 0 && is_projectile == false)
                Position.Y = 0;
            if ((Position.Y + Size.Height) >= MAX_WINY && is_projectile == false)
